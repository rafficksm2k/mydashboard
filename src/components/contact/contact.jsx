...
export default function contact() {
  ...
  const nodeSendEmail = async () => {
    ...
    try {
      ...
      console.error("something failed in node js API call: ", err);
    }
  };
    ...
    <Container>
      <Row>
        <Col className="contact" xs="12" sm="12" md="6">
          <strong className="main-name purple contact-header">
            <h1>Contact Me</h1>
          </strong>
          {!successPage ? (
            <Form ref={form}>
              {formDetails.map((detail, index) => (
                ...
                >
                  <ContactFormLabel
                    text={detail.label}
                    classNme={detail.labelClassName}
                  />
                  <Form.Control
                    name={detail.inputName}
                  ...
                  {!detail.isValid ? (
                    <ContactFormLabel
                      text={detail.errorLabel}
                      classNme={detail.errorLabelClassName}
                    />
                  ) : null}
                </Form.Group>
              ))}
